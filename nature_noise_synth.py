...
class NoiseGenerator:
    ...
    def pink_noise(self, num_samples):
        ...
        for i in range(num_samples):
            ...
            diff = 0
            for j in range(16):
                if (self.pink_key >> j) & 1:
                    if j == 0 or not ((self.pink_key >> (j-1)) & 1):
                        self.pink_state[j] = np.random.randn()
                    diff = j
                    break
            
            # Sum all active generators
            pink[i] = np.sum(self.pink_state[:diff+1]) / 8
    ...
    def brown_noise(self, num_samples):
        ...
        for i in range(num_samples):
            white = np.random.randn() * 0.1
            self.brown_last = (self.brown_last + white) * 0.998  # Slight decay to prevent drift
    ...
    def blue_noise(self, num_samples):
        ...
        # Simple high-pass filter to create blue noise
        blue = np.zeros(num_samples)
        for i in range(1, num_samples):
            blue[i] = white[i] - white[i-1] * 0.9
        return blue

class ResonantFilter:
    """Multi-mode resonant filter for shaping noise"""
    ...
    def process(self, input_signal, cutoff_mod=None):
        """Process signal through resonant filter"""
        output = np.zeros_like(input_signal)
        
        for i, sample in enumerate(input_signal):
            # Apply cutoff modulation if provided
            if cutoff_mod is not None and i < len(cutoff_mod):
                current_cutoff = self.cutoff * cutoff_mod[i]
            else:
                current_cutoff = self.cutoff
                
            # Clamp cutoff to valid range
            current_cutoff = max(20, min(20000, current_cutoff))
            
            # Calculate filter coefficients (improved stability)
            f = 2 * np.sin(np.pi * min(0.25, current_cutoff / SAMPLE_RATE))  # Reduced from 0.48
            q = max(0.5, min(1.5, self.resonance))  # Reduced max resonance
            scale = 1.0 / (1.0 + q)  # Better scaling
            
            # State variable filter with stability checks
            self.band = self.band + f * self.high
            self.low = self.low + f * self.band
            self.high = scale * (sample - self.low - q * self.band)
            
            # Prevent NaN/infinity
            if np.isnan(self.low) or np.isinf(self.low):
                self.low = 0.0
            if np.isnan(self.high) or np.isinf(self.high):
                self.high = 0.0
            if np.isnan(self.band) or np.isinf(self.band):
                self.band = 0.0
                
            # Soft clipping to prevent overflow
            self.low = np.tanh(self.low)
            self.high = np.tanh(self.high)
            self.band = np.tanh(self.band)
...
class BirdSound:
    """Bird chirping sound generator using filtered noise bursts - tuned to 432Hz harmonics"""
    
    def __init__(self):
        self.noise_gen = NoiseGenerator()
        # Bird calls tuned to 432Hz harmonic series
        base_432 = 432.0 * 4  # 1728Hz - 2 octaves above A432
        self.chirp_filter = ResonantFilter(base_432, 2.0, 'bandpass')
        self.amplitude = 0.15
        self.active = True
        
        # Chirp timing
        self.chirp_timer = 0
        self.chirp_interval = SAMPLE_RATE * random.uniform(2, 5)  # 2-5 seconds
        self.chirp_duration = int(SAMPLE_RATE * 0.1)  # 100ms chirps
        self.chirp_phase = 0
        self.chirp_buffer = np.zeros(256)  # Small buffer for chirp generation
        
        # Modulation
        self.freq_sweep = LFO(20, 0.3, 'sine')  # Fast sweep for chirp
    ...
    def generate(self, num_samples):
        ...
        # Generate in smaller chunks for stability
        chunk_size = 64
        for chunk_start in range(0, num_samples, chunk_size):
            chunk_end = min(chunk_start + chunk_size, num_samples)
            chunk_len = chunk_end - chunk_start
            
            # Check if we should start a new chirp
            if self.chirp_timer >= self.chirp_interval:
                self.chirp_timer = 0
                self.chirp_interval = SAMPLE_RATE * random.uniform(1.5, 4)
                self.chirp_phase = 0
                
            # Generate chirp for this chunk
            for i in range(chunk_len):
                idx = chunk_start + i
                self.chirp_timer += 1
                
                if self.chirp_phase < self.chirp_duration:
                    # Chirp envelope
                    env = np.sin(np.pi * self.chirp_phase / self.chirp_duration)
                    
                    # Generate noise burst
                    noise = np.random.randn() * env * 0.5
                    
                    # Frequency modulation (432Hz harmonics)
                    freq_mod = 1.0 + np.sin(2 * np.pi * 30 * self.chirp_phase / SAMPLE_RATE) * 0.5
                    self.chirp_filter.cutoff = 1728 + 864 * freq_mod  # 432Hz * 4 ± 432Hz * 2
                    
                    # Process through filter
                    filtered = self.chirp_filter.process(np.array([noise]), np.array([1.0]))
                    output[idx] = filtered[0] * self.amplitude
    ...
    def audio_callback(self, outdata, frames, time_info, status):
        ...
        if self.auto_evolve:
            ...
            if self.evolution_timer >= self.evolution_interval:
                self.evolution_timer = 0
                self.evolution_interval = SAMPLE_RATE * random.uniform(20, 40)
                self.evolve_scene()
        
        # Smooth amplitude transitions
        for key in self.current_amps:
            diff = self.target_amps[key] - self.current_amps[key]
            self.current_amps[key] += diff * 0.001  # Smooth transition
            
        # Generate sounds from each voice
        ocean_sound = self.ocean.generate(frames) * self.current_amps['ocean']
        wind_sound = self.wind.generate(frames) * self.current_amps['wind']
        rain_sound = self.rain.generate(frames) * self.current_amps['rain']
        fire_sound = self.fire.generate(frames) * self.current_amps['fire']
        bird_sound = self.birds.generate(frames) * self.current_amps['birds']
        ambience_sound = self.ambience.generate(frames) * self.current_amps['ambience']
        
        # Mix all sounds
        mixed = ocean_sound + wind_sound + rain_sound + fire_sound + bird_sound + ambience_sound
        
        # Create stereo image
        left = mixed.copy()
        right = mixed.copy()
        
        # Apply different panning to each element for width
        pan_ocean = 0.3 + self.ocean.pan_lfo.generate(frames)[0] * 0.2
        pan_wind = 0.7 + self.wind.pan_lfo.generate(frames)[0] * 0.2
        
        left *= (1 - pan_ocean) + (1 - pan_wind) * 0.5
        right *= pan_ocean + pan_wind * 0.5
        
        # Apply reverb for spaciousness
        for i in range(frames):
            # Read from reverb buffer
            delay_pos1 = (self.reverb_index - int(SAMPLE_RATE * 0.07) + i) % self.reverb_buffer_size
            delay_pos2 = (self.reverb_index - int(SAMPLE_RATE * 0.13) + i) % self.reverb_buffer_size
            
            reverb_l = self.reverb_buffer_l[delay_pos1] * 0.3 + self.reverb_buffer_l[delay_pos2] * 0.2
            reverb_r = self.reverb_buffer_r[delay_pos1] * 0.3 + self.reverb_buffer_r[delay_pos2] * 0.2
            
            # Add reverb with cross-feeding
            left[i] += reverb_r * self.reverb_mix
            right[i] += reverb_l * self.reverb_mix
            
            # Update reverb buffer
            write_pos = (self.reverb_index + i) % self.reverb_buffer_size
            self.reverb_buffer_l[write_pos] = left[i] * 0.5
            self.reverb_buffer_r[write_pos] = right[i] * 0.5
            
        self.reverb_index = (self.reverb_index + frames) % self.reverb_buffer_size
